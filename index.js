...
// Types
// Transpiled types are just comments in JS, they don't affect runtime.
...
const bloodTests = {
    ...
    "URINE (ACR)": { "Diabetes Mellius": "Annually", "Chronic Kidney Disease": "Annually" }
};

// Logic
const IS_MONITORING_EXCLUSIONS_INCLUDE = ["diagnosis", "to check cvd risk", "make diagnosis", "no monitoring", "after starting treatment"];
const IS_MONITORING_EXCLUSIONS_EXACT = [];

const isMonitoringFrequency = (freq) => {
...
const getCKDFrequency = (stage, test) => {
    if (test === "U&Es") {
        switch (stage) {
...
const calculateRequiredTests = (formState) => {
    // Step 1: Aggregate all test data first.
    const requiredTestsData = {};
        ...
        selectedDiseases.forEach(disease => {
            if (conditionsForTest.hasOwnProperty(disease)) {
                ...
                if (typeof rawFrequencyEntry === 'string') {
                    ...
                    const conditionMet = formState[rawFrequencyEntry.conditionKey];
...
const InfoPopup = ({ messageHtml, style, onConfirm }) => {
    ...
    return (
        React.createElement("div", { className: "clinical-popup", style: style, ref: rootDivRef, dangerouslySetInnerHTML: { __html: INFO_POPUP_HTML } })
    );
};
...
const Header = ({ isCompact }) => {
    ...
    React.useEffect(() => {
        ...
        const handleClickOutside = (event) => {
            ...
            if (
                showInfoPopup &&
                overlayRef.current && overlayRef.current.contains(event.target) && // Click is within the overlay
                !infoButtonRef.current?.contains(event.target) // And not on the info button
            ) {
                ...
                // This simplifies external click handling since the whole overlay is tied to closing.
                 if (!(event.target).closest('.clinical-popup')) {
    ...
    return (
        React.createElement("header", { className: `app-header ${isCompact ? 'compact' : ''}` },
            React.createElement("div", { className: "header-title-group" },
                React.createElement("div", { className: "header-icon", dangerouslySetInnerHTML: { __html: medicalCrossIcon } }),
                React.createElement("h1", null, "HGP Blood Test Allocator")
            ),
            React.createElement("button", { 
                ...
                ref: infoButtonRef
                 ...
                 React.createElement("div", { dangerouslySetInnerHTML: { __html: infoIcon } })
            ),
            showInfoPopup && (
                React.createElement(React.Fragment, null,
                    React.createElement("div", { className: "popup-overlay", ref: overlayRef }),
...
const SelectionCard = ({ formState, handleChange, onReset, hasSelections }) => {
    const isQuestionVisible = (disease) => formState.selectedDiseases.has(disease.name) && disease.questions;

    return (
        React.createElement("div", { className: "card" },
            React.createElement("div", { className: "card-header" },
                React.createElement("span", null, "Select Conditions"),
                React.createElement("button", { className: "button-reset", onClick: onReset, disabled: !hasSelections }, "Reset")
            ),
            React.createElement("ul", { className: "list-group" },
                diseases.map((disease) => (
                    React.createElement(React.Fragment, { key: disease.name },
                        React.createElement("li", { className: "list-item" },
                            React.createElement("div", { className: "list-item-content" },
                                React.createElement("div", { className: `list-item-icon ${disease.colorClass}` },
                                    disease.name.split(' ').map(w => w[0]).join('').substring(0, 3)
                                ),
                                React.createElement("span", { className: "list-item-label" }, disease.name)
                            ),
                        ...
                        isQuestionVisible(disease) && disease.questions?.map(q => (
                             React.createElement("li", { key: q.key, className: `additional-info-item ${isQuestionVisible(disease) ? 'visible' : ''}` },
                                React.createElement("label", { htmlFor: q.key }, q.label),
                                q.type === 'toggle' && (
                                     React.createElement("label", { className: "toggle-switch" },
                                         React.createElement("input", { type: "checkbox", checked: !!formState[q.key], onChange: e => handleChange(q.key, e.target.checked, 'toggle') }),
                                         React.createElement("span", { className: "slider" })
                                     )
                                ),
                                q.type === 'select' && q.options && (
                                     React.createElement("div", { className: "select-wrapper" },
                                        React.createElement("select", { id: q.key, value: formState[q.key], onChange: e => handleChange(q.key, e.target.value, 'select') },
                                             q.options.map(opt => React.createElement("option", { key: opt.value, value: opt.value }, opt.label))
                                         )
                                     )
                                )
                             )
...
const ResultsCard = ({ results }) => {
    if (results.length === 0) {
        return (
            React.createElement("div", { className: "card" },
                React.createElement("div", { className: "card-header" }, "Required Tests"),
                React.createElement("div", { className: "results-placeholder" },
                    React.createElement("div", { className: "placeholder-icon", dangerouslySetInnerHTML: { __html: animatedPlaceholderIcon } }),
                    React.createElement("p", null, "Select one or more conditions to see the required blood tests.")
                )
            )
        );
    }
    ...
    return (
        React.createElement("div", { className: "card" },
            React.createElement("div", { className: "card-header" }, "Required Tests"),
            React.createElement("ul", { className: "results-list", key: listKey },
                results.map((test, index) => {
                    ...
                    return (
                        React.createElement("li", { className: "result-item", key: test.testName, style: { animationDelay: `${index * 50}ms` } },
                            React.createElement("span", { className: "test-name" }, test.testName),
                            monitoringFreqs.length > 0 && (
                                React.createElement("div", { className: "result-category" },
                                    React.createElement("div", { className: "category-header" },
                                        React.createElement("div", { className: "category-icon monitoring-icon", dangerouslySetInnerHTML: { __html: sparkleIcon } }),
                                        React.createElement("span", null, "Monitoring")
                                    ),
                                    React.createElement("div", { className: "frequencies-container" },
                                        monitoringFreqs.map((frequency, freqIndex) => (
                                            React.createElement("div", { className: "frequency-group", key: freqIndex },
                                                React.createElement("span", { className: `frequency-tag ${getFrequencyColor(frequency.frequency)}` }, frequency.frequency),
                                                React.createElement("span", { className: "disease-info" }, "for ", frequency.diseases.join(', '))
                                            )
                                        ))
                                    )
                                )
                            ...
                            diagnosticFreqs.length > 0 && (
                                React.createElement("div", { className: "result-category" },
                                    React.createElement("div", { className: "category-header" },
                                        React.createElement("div", { className: "category-icon diagnostic-icon", dangerouslySetInnerHTML: { __html: exclamationIcon } }),
                                        React.createElement("span", null, "Diagnostic / Other")
                                    ),
                                    React.createElement("div", { className: "frequencies-container" },
    ...
    React.createElement("footer", { className: "app-footer" },
        React.createElement("p", null,
            React.createElement("span", { className: "footer-text" },
                ...
                ' ',
                React.createElement("a", { href: "https://www.harwoodgrouppractice.co.uk", target: "_blank", rel: "noopener noreferrer" }, "Harwood Group Practice")
            )
        )
    )
);
...
const App = () => {
    ...
    const calculatedTests = React.useMemo(() => calculateRequiredTests(formState), [formState]);
    ...
    return (
        React.createElement(React.Fragment, null,
            React.createElement(Header, { isCompact: isCompact }),
            React.createElement("div", { className: "app-container" },
                React.createElement("main", { className: "main-content" },
                    React.createElement(SelectionCard, { formState: formState, handleChange: handleChange, onReset: handleReset, hasSelections: hasSelections }),
                    React.createElement(ResultsCard, { results: calculatedTests })
                )
            ),
